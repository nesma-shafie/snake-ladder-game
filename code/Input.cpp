...
int Input::GetInteger(Output *pO) const 
{
	...
	return integer; //     done                  this line should be changed with your implementation
}
...
{	
	...
	else	
	{
		...
		if (y >= 0 && y < UI.ToolBarHeight)
		{
			...
			case ITM_Input_Dice_Value:
				return Input_Dice_Value;
			case ITM_Exit:
				return Exit;
			case ITM_New_Game:
				return New_Game;
			...
			return STATUS;	// just for now ==> This should be updated//UPDATED
...
CellPosition Input::GetCellClicked() const
{
	...
	if ( UI.InterfaceMode == MODE_DESIGN )	
	{
		if ( y >= UI.ToolBarHeight && y <= (UI.height - UI.StatusBarHeight))
		{
			...
			int H=0, V=0;
			do {
				if (!(y <= UI.CellHeight && y >= 0)) {
					y = y-UI.CellHeight;
				}
				for (; !(y <= UI.CellHeight && y >= 0);y-= UI.CellHeight) {
					V++;
				}
				for (; !(x <= UI.CellWidth && x >= 0);x-= UI.CellWidth) {
					H++;
				}
			} while (!((x <= UI.CellWidth && x >= 0) && (y <= UI.CellHeight && y >= 0)));
