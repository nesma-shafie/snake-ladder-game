...
Output::Output()
{
	...
	//////////////////////////////////////////////////////
	//UI.CellScaleH = UI.width / NumHorizontalCells;
	//UI.CellScaleV= (UI.height - UI.ToolBarHeight - UI.StatusBarHeight) / NumVerticalCells;
	/////////////////////////////////////////
	...
	UI.CellColor_NoCard = CORNFLOWERBLUE;//LIGHTSLATEBLUE;
	UI.CellNumFont = 13;
...
int Output::GetCellStartY(const CellPosition & cellPos) const
{
	...
	return (UI.CellHeight * cellPos.VCell() + UI.CellHeight); // this line should be changed with your implementation
}
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX, cellStartY,cellStartX+UI.CellWidth,cellStartY +UI.CellHeight, FILLED,10,10);
	...
	int w=0, h=0;
	//pWind->DrawInteger(cellStartX + UI.CellScaleH -w, cellStartY + UI.CellScaleV - h, cellNum);
	
	//pWind->DrawInteger(cellStartX + 90, cellStartY + 40, cellPos.GetCellPositionFromNum(50).HCell());
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if (playerNum < 4 && playerNum >= 0) {
		...
		int radius = (UI.CellWidth / UI.CellWidth)*8 ; // proportional to cell width

		// Calculate the horizontal space before drawing players circles (space from the left border of the cell)
		int ySpace = UI.CellHeight / 27; // proportional to cell height
...
void Output::DrawLadder(const CellPosition& fromCell, const CellPosition& toCell) const
{
	...
	if (fromCell.VCell() > toCell.VCell() && fromCell.HCell() == toCell.HCell() && fromCell.VCell() != 0) {
		...
		for (int i = y1 - 28;i >= y2;i = i - UI.CellHeight)
			pWind->DrawLine(x34 , i, x12, i, FRAME);
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	if (fromCell.VCell() < toCell.VCell() && fromCell.HCell() == toCell.HCell() ) {
		...
		int yChange = fromStartY; // slight vertical difference to be used in the up and down polygon points
		int xChange = x12; // slight horizontal difference to be used in the left and right polygon points
		...
		int x[4] = {x12,x12+7,x12,x12-7};
		int y[4] = {yChange+20,yChange +20+10,yChange +20+20,yChange +20+10};
